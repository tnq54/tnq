...
<body>
    ...
    <div class="main-layout">
        ...
        <div>
            ...
            <!-- Neurotransmitter Gauges -->
            <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; margin-bottom: 20px;">
                <div style="background: #0A0B0C; border: 1px solid rgba(255,255,255,0.08); border-radius: 6px; padding: 10px; text-align: center;">
                    <div style="font-size: 0.7rem; color: #9CA3AF;">ACh (Acetylcholine)</div>
                    <div style="font-size: 1.1rem; font-weight: 700; color: #00F0FF;" id="ach-val">75.0%</div>
                </div>
                <div style="background: #0A0B0C; border: 1px solid rgba(255,255,255,0.08); border-radius: 6px; padding: 10px; text-align: center;">
                    <div style="font-size: 0.7rem; color: #9CA3AF;">5-HT (Serotonin)</div>
                    <div style="font-size: 1.1rem; font-weight: 700; color: #FF007F;" id="sht-val">80.0%</div>
                </div>
                <div style="background: #0A0B0C; border: 1px solid rgba(255,255,255,0.08); border-radius: 6px; padding: 10px; text-align: center;">
                    <div style="font-size: 0.7rem; color: #9CA3AF;">END (Endorphin)</div>
                    <div style="font-size: 1.1rem; font-weight: 700; color: #FFD700;" id="end-val">60.0%</div>
                </div>
            </div>
            ...
            <div class="card">
                ...
                <div style="margin-top: 10px; display: flex; gap: 8px; justify-content: space-between; align-items: center; flex-wrap: wrap;">
                    <div style="display: flex; gap: 6px; align-items: center;">
                        <span style="font-size: 0.75rem; color: #9CA3AF;">Giao diện:</span>
                        <select id="theme-select" style="background: #0A0B0C; color: #FFF; border: 1px solid rgba(255,255,255,0.15); border-radius: 4px; font-size: 0.75rem; padding: 3px 6px;" onchange="setVisualTheme(this.value)">
                            <option value="Neon">⚡ Neon Bio</option>
                            <option value="Cyberpunk">🌆 Cyberpunk 2077</option>
                            <option value="BioRed">🩸 Bio-Red Vascular</option>
                        </select>
                    </div>
                    <button class="btn" id="vasc-btn" style="width: auto; font-size: 0.75rem; padding: 4px 10px; background: #8b0000; border-color: #ff3333;" onclick="toggleVascularMesh()">🩸 Mạch Máu: Bật</button>
                    <div style="display: flex; gap: 4px; align-items: center;">
                        <span style="font-size: 0.75rem; color: #9CA3AF;">Góc camera:</span>
                        <button class="btn" style="width: auto; font-size: 0.7rem; padding: 3px 6px;" onclick="setCameraView('iso')">📐 3D Iso</button>
                        <button class="btn" style="width: auto; font-size: 0.7rem; padding: 3px 6px;" onclick="setCameraView('top')">⬇️ Top</button>
                        <button class="btn" style="width: auto; font-size: 0.7rem; padding: 3px 6px;" onclick="setCameraView('front')">👁️ Front</button>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        let showVascularMesh = true;

        let acetylcholine = 75.0;
        let serotonin = 80.0;
        let endorphin = 60.0;
        let microgliaActive = 4;

        let dopamineRewardLevel = 0.0;
        let pathologyAlzheimer = false;
        let pathologyEpilepsy = false;
        let pathologySchizophrenia = false;
        let pathologyParkinson = false;
        let pathologyADHD = false;
        let drugLDOPA = false;
...
fn main() {
    ...
    gaba[0] = 90.0;
        ...
        function drawLoop() {
            ...
            // Draw 3D Vascular Capillary Mesh System
            if (showVascularMesh) {
                let vascPulse = Math.sin(Date.now() * 0.003) * 0.1 + 0.25;
                ctx.lineWidth = 1.2;
                ctx.strokeStyle = `rgba(220, 20, 60, ${vascPulse})`;
                for (let i = 0; i < nodesProjected.length - 1; i += 2) {
                    let n1 = nodesProjected[i];
                    let n2 = nodesProjected[i + 1];
                    ctx.beginPath();
                    ctx.moveTo(n1.px, n1.py);
                    let cpX = (n1.px + n2.px) / 2 + Math.sin(i) * 15;
                    let cpY = (n1.py + n2.py) / 2 + Math.cos(i) * 15;
        ...
        function simTick() {
            ...
            // --- Microglia Immune Cleanup & Neurotransmitter Synthesis ---
            acetylcholine = Math.min(100.0, Math.max(10.0, acetylcholine + (activeCharged > 2 ? 0.8 : -0.2)));
            serotonin = Math.min(100.0, Math.max(10.0, serotonin + (sanity > 50 ? 0.5 : -0.3)));
            endorphin = Math.min(100.0, Math.max(10.0, endorphin + (stress < 30 ? 0.4 : -0.2)));

            // Microglia immune patrolling
            if (microgliaActive > 0 && currentTick % 5 === 0) {
                vpi = Math.min(100.0, vpi + 0.5);
                if (pathologyAlzheimer) {
                    addLog(`🛡️ [Microglia Immune] Tế bào miễn dịch Microglia đã dọn dẹp độc tố Amyloid Plaque, bảo vệ vpi lên ${vpi.toFixed(1)}%.`);
                }
            }
            ...
            document.getElementById("ach-val").textContent = `${acetylcholine.toFixed(1)}%`;
            document.getElementById("sht-val").textContent = `${serotonin.toFixed(1)}%`;
            document.getElementById("end-val").textContent = `${endorphin.toFixed(1)}%`;
